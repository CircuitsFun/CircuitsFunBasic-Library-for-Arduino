...
LCD::LCD(uint8_t addr, uint8_t lcd_cols,uint8_t lcd_rows) {
  _Addr = addr;
  _cols = lcd_cols;
  _rows = lcd_rows;
...
void LCD::init(uint8_t addr,uint8_t lcd_cols,uint8_t lcd_rows){
  _Addr = addr;
  _cols = lcd_cols;
  _rows = lcd_rows;
  _backlightval = LCD_NOBACKLIGHT;
...
void LCD::printLCD(const char *word, int offset, int line) {
	setCursor(offset, line);
	printstr(word);
}

void LCD::printLCD(String word, int offset, int line) {
	setCursor(offset, line);
	printstr(word.c_str());
}
